import { getAllTasks, getTaskByState, changeTasksState, createTask, updatedTask, deleteTask } from '../models/taskModels.js';
...
export const updateTask = async (req, res) => {
    ...
    if (!taskId || !title || !description) {
        return res.status(400).json({ error: 'Task ID, title, and description are required for update' });
    }

    try {
        const updated = await updatedTask({ id: taskId, title, description });
        res.status(200).json({ message: `Task ID ${taskId} updated successfully`, updated });
    } catch (error) {
    ...
    try {
        switch (state) {
            ...
            case 'Done':
                ...
                break;
            default:
                return res.status(400).json({ error: `Invalid state '${state}'. Valid states are: 'To Do', 'In Progress', 'Done'` });
        }
    } catch (error) {
        res.status(500).json({ error: `Error retrieving tasks with state '${state}'`, details: error.message });
    }
};
...
export const changeStates = async (req, res) => {
    ...
    try {
       if (!id) {
            return res.status(400).json({ error: 'Task ID is required to change state' });
        }
        const changedState = await changeTasksState(parseInt(id));
        if (changedState) {
            res.status(200).json({ message: `Task ID ${id} state changed successfully`, changedState });
        } else {
            res.status(404).json({ error: `Task ID ${id} doesn't have the expected state` });
        }
    } catch (error) {
        res.status(500).json({ error: `Error changing state for task ID ${id}`, details: error.message });
    }
}